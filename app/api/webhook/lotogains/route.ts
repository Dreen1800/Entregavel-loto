...
export async function POST(request: NextRequest) {
  try {
    console.log('🎯 Webhook LotoGains 10X recebido')
    ...
    console.log(`🎯 Ativando LotoGains 10X para: ${data.name} (${data.email})`)
    
    // Ativar LotoGains 10X para o usuário
    const result = await activateLotoGains10x(data.email)

    if (result.success) {
      console.log('✅ LotoGains 10X ativado com sucesso:', data.email)
      return NextResponse.json(
        { 
          message: 'LotoGains 10X activated successfully',
          user: result.user
        },
        { status: 200 }
      )
    } else {
      console.error('❌ Erro ao ativar LotoGains 10X:', result.error)
      return NextResponse.json(
        { 
          error: 'Failed to activate LotoGains 10X',
    ...
    console.error('💥 Erro no webhook LotoGains 10X:', error)
...
// Ativar LotoGains 10X para o usuário
async function activateLotoGains10x(email: string) {
  try {
    const supabase = createWebhookClient()
    
    console.log('🎯 Ativando LotoGains 10X para:', email)
    ...
    console.log('✅ LotoGains 10X ativado com sucesso')
    return { success: true, user: updatedUser }

  } catch (error: any) {
    console.error('💥 Erro ao ativar LotoGains 10X:', error)
    return { 
      success: false, 
      error: error.message,
      code: error.code 
...
export async function GET() {
  return NextResponse.json(
    { message: 'LotoGains 10X webhook endpoint - POST only' },
    { status: 405 }
  )
}